...
app.get("/users", (req, res) => {
  ...
  if (req.query.id || req.query.name) {
    ...
    const user = usersData.find(
      (user) => user.id == req.query.id || user.name == req.query.name
    );
    if (!user) {
      res.status(404).json({
        massage: "User's data",
    ...
    res.json({
      massage: "User get api",
      sussess: true,
      result: [usersData],
    });
  }
});
app.post("/todos/:id/:name", (req, res) => {
  console.log(req.params);
  const todos = todosData.filter((todo) => todo.id == req.params.id);
  res.json({
    massage: "todo get api",
    result: todos,
  });
});
